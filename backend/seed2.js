...
const seedDatabase = async () => {
  try {
    ...
    // 1️⃣ SEED USERS (Expanded User Registry)
    ...
    const users = await User.create([
      ...
      { username: 'winter_shield', email: 'winter@echo.com', password: 'password123', role: 'customer' },
      { username: 'charlie_blink', email: 'charlie@echo.com', password: 'password123', role: 'customer' },
    ...
    // 3️⃣ SEED VENUES (Expanded Global Arena Inventory)
    ...
    const venues = await Venue.create([
      ...
      },
      {
        ...
        country: 'United States',
        capacity: 19000,
        location: { type: 'Point', coordinates: [-118.2673, 34.0430] }
      }
    ]);

    // 4️⃣ PRE-GENERATE OBJECTIDS FOR EVERY EMBEDDED CONCERT
    // This allows us to link tickets directly to a specific subdocument without data mapping drift
    const aespaConcert1 = new mongoose.Types.ObjectId();
    const aespaConcert2 = new mongoose.Types.ObjectId();
    
    const iuConcert = new mongoose.Types.ObjectId();
    
    const njConcert1 = new mongoose.Types.ObjectId();
    const njConcert2 = new mongoose.Types.ObjectId();
    const njConcert3 = new mongoose.Types.ObjectId();
    
    const skzConcert1 = new mongoose.Types.ObjectId();
    const skzConcert2 = new mongoose.Types.ObjectId();
    
    const taeyeonConcert = new mongoose.Types.ObjectId();

    // 5️⃣ SEED TOURS & MULTI-STOP CONCERTS
    ...
    const tours = await Tour.create([
      {
        ...
        concerts: [
          ...
          {
            concertId: aespaConcert2,
            venue: venues.find(v => v.name === 'Kyocera Dome')._id,
            date: new Date('2025-08-13T18:00:00Z'),
            availableTickets: 0,
            ticketPrice: 135,
            status: 'Sold Out',
            setlist: ['Drama', 'Savage', 'Supernova', 'Spicy', 'Armageddon'],
            assignedStaff: [managerId]
          }
        ]
      },
      ...
      },
      {
        ...
        concerts: [
          ...
          {
            concertId: njConcert2,
            venue: venues.find(v => v.name === 'The O2 Arena')._id,
            date: new Date('2026-07-22T20:00:00Z'),
            availableTickets: 18500, 
            ticketPrice: 160,
            status: 'Scheduled',
            setlist: ['Attention', 'Hype Boy', 'Cookie', 'Ditto', 'Super Shy', 'Bubble Gum'],
            assignedStaff: [managerId]
          },
          {
            concertId: njConcert3,
            venue: venues.find(v => v.name === 'Madison Square Garden')._id,
            date: new Date('2026-07-28T19:30:00Z'),
            availableTickets: 19500, 
            ticketPrice: 170,
            status: 'Scheduled',
            setlist: ['Attention', 'Hype Boy', 'Ditto', 'OMG', 'Super Shy', 'How Sweet'],
            assignedStaff: [managerId]
      ...
      },
      {
        ...
        concerts: [
          ...
          {
            concertId: skzConcert2,
            venue: venues.find(v => v.name === 'Crypto.com Arena')._id,
            date: new Date('2026-09-18T19:00:00Z'),
            availableTickets: 19000,
            ticketPrice: 180,
            status: 'Scheduled',
            setlist: ['S-Class', 'MANIAC', 'Back Door', 'LALALALA', 'Chk Chk Boom'],
            assignedStaff: [managerId]
          }
        ]
      },
    ...
    // 6️⃣ SEED METRICS TICKETS (Broad dataset for operational financial layout metrics)
    console.log('🎟 * Seeding Financial Tickets...');
    await Ticket.create([
      {
        user: customerIds[0],
      ...
      },
      {
        ...
        concertId: aespaConcert2,
        seatCategory: 'Standing',
        purchaseDate: new Date('2025-06-03T14:20:00Z'),
        status: 'Confirmed',
        totalPaid: 135
      },
      {
      ...
      },
      {
        ...
        concertId: njConcert2,
        seatCategory: 'Standing',
        purchaseDate: new Date('2026-05-14T11:00:00Z'),
        status: 'Confirmed',
        totalPaid: 160
      }
    ]);

    console.log('🏁 Success! Master database compiled with original & scalable data structures.');
    process.exit(0);
  } catch (error) {
